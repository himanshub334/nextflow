...
async function executeWorkflow({
  runId, nodes, edges, mode, selectedNodeIds,
}: {
  runId: string; nodes: AppNode[]; edges: AppEdge[];
  mode: ExecutionMode; selectedNodeIds?: string[];
}) {
  ...
  // Pre-populate static node outputs
  ...
  for (const level of levels) {
    await Promise.all(level.map(async (nodeId) => {
      ...
      // Static nodes — already pre-populated
      if (['text', 'uploadImage', 'uploadVideo'].includes(node.data.type)) {
        nodeResults.push({
          nodeId, nodeType: node.data.type, nodeLabel: node.data.label,
      ...
      try {
        ...
        nodeResults.push({
          nodeId, nodeType: node.data.type, nodeLabel: node.data.label,
          status: 'success', inputs: edgeInputs, output,
