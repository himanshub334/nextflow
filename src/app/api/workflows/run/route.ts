...
import { runLLMTask, cropImageTask, extractFrameTask } from '@trigger.dev/sdk/v3';
...
export async function POST(req: NextRequest) {
  ...
  // Execute async — return runId immediately for polling
  executeWorkflow({
    runId: run.id,
...
async function executeWorkflow({
  ...
  const targetIds =
    mode === 'single' && selectedNodeIds?.length === 1
      ? selectedNodeIds
      : mode === 'selected' && selectedNodeIds?.length
      ? selectedNodeIds
      : undefined;
  ...
  // Pre-populate outputs from static nodes (Text, Upload)
  for (const node of nodes) {
    if (node.data.type === 'text') {
      nodeOutputs[node.id] = node.data.text;
    } else if (node.data.type === 'uploadImage') {
      nodeOutputs[node.id] = node.data.imageUrl;
    } else if (node.data.type === 'uploadVideo') {
      nodeOutputs[node.id] = node.data.videoUrl;
    }
  }
  ...
  for (const level of levels) {
    ...
    await Promise.all(
      level.map(async (nodeId) => {
        ...
        // Skip static nodes in execution
        if (
          node.data.type === 'text' ||
          node.data.type === 'uploadImage' ||
          node.data.type === 'uploadVideo'
        ) {
          nodeResults.push({
            nodeId,
        ...
        try {
          ...
          if (node.data.type === 'llm') {
            const imageUrls = [edgeInputs['images'], ...(Array.isArray(edgeInputs['images']) ? edgeInputs['images'] : [])].filter(Boolean) as string[];
            const taskRun = await runLLMTask.trigger({
            ...
            const taskRun = await extractFrameTask.trigger({
